...
/**
 * Confirmed Continuation Strategy — Bootstrap
 *
 * Loads and merges config (base + active overrides), validates against
 * schema.php, and returns the effective config.  Called by
 * ConfirmedContinuationService before any runtime logic.
 * Throws \RuntimeException on invalid config.
 */

namespace Modules\Strategy\ConfirmedContinuation;

final class ConfirmedContinuationBootstrap
{
    ...
    {
        if (!file_exists($path)) {
            throw new \RuntimeException("Confirmed Continuation config file missing [{$label}]: {$path}");
        }
        $data = require $path;
        if (!is_array($data)) {
            throw new \RuntimeException("Confirmed Continuation config file [{$label}] must return array: {$path}");
        }
        return $data;
    }
