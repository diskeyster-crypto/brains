...
final class DoubleBottomLongBootstrap
{
    ...
    {
        if (!file_exists($path)) {
            throw new \RuntimeException("Double Bottom Long config file missing [{$label}]: {$path}");
        }
        $data = require $path;
        if (!is_array($data)) {
            throw new \RuntimeException("Double Bottom Long config file [{$label}] must return array: {$path}");
        }
        return $data;
    }
