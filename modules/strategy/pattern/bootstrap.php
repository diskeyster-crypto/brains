...
final class PatternBootstrap
{
    ...
    {
        if (!file_exists($path)) {
            throw new \RuntimeException("Pattern config file missing [{$label}]: {$path}");
        }
        $data = require $path;
        if (!is_array($data)) {
            throw new \RuntimeException("Pattern config file [{$label}] must return array: {$path}");
        }
        return $data;
    }
