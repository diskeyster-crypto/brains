...
final class BotBootstrap
{
    ...
    {
        if (!file_exists($path)) {
            throw new \RuntimeException("Bot config file missing [{$label}]: {$path}");
        }
        $data = require $path;
        if (!is_array($data)) {
            throw new \RuntimeException("Bot config file [{$label}] must return array: {$path}");
        }
        return $data;
    }
