...
/**
 * Controlled Daily Momentum Long — Service
 *
 * Thin orchestration layer called by CronManager.
 * Delegates all work to ControlledDailyMomentumLongStrategy.
 *
 * Handler:
...
final class ControlledDailyMomentumLongService
{
     ...
     * Runs the full controlled-daily-momentum-long scan cycle.
     */
    public function tickRun(): void
    {
        try {
            $strategy = new ControlledDailyMomentumLongStrategy($this->moduleDir);
            $result   = $strategy->run();
            ...
            if (!($result['ok'] ?? false)) {
                ...
                if ($error !== 'strategy_disabled') {
                    Logger::cron('controlled_daily_momentum_long tickRun error', ['error' => $error]);
                }
                return;
            }

            $stats = $result['stats'] ?? [];
            Logger::cron('controlled_daily_momentum_long tickRun completed', [
            ...
            Logger::cron('controlled_daily_momentum_long tickRun exception', ['error' => $e->getMessage()]);
        }
    }
}
